        ...
        for link in item.get("links", []):
            ...
            if linked_coords:
                ...
                if connection_key not in seen_connections:
                    connections.append(
                        (current_id, linked_id, current_coords, linked_coords)
                    )
...
def create_folium_map(
    metadata: List[Dict], center_coords: Tuple[float, float], connections: List[Tuple]
) -> folium.Map:
...
def visualize_points_on_map(jsonl_file_path: str, output_html_path: str = None):
    ...
    print(
        f"Map saved successfully! Open {output_html_path} in your browser to view the visualization."
    )
    print(
        "Use the layer control in the top-right to toggle points and connections visibility."
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(metadata_file):
        ...
        print(
            "Make sure you're running this script from the experimentalloading directory"
        )
    else:
